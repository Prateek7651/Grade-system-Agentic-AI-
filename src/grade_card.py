...
GRADE_COLORS = {
    'A+': colors.HexColor('#0F6E56'), 'A': colors.HexColor('#1F6FB2'),
    'B': colors.HexColor('#B8860B'), 'C': colors.HexColor('#D2691E'), 'F': colors.HexColor('#B22222'),
}
...
def _build_styles():
    ...
    styles.add(ParagraphStyle(name='InstituteTitle', fontSize=16, leading=20, alignment=TA_CENTER, textColor=colors.HexColor('#1F4E78'), fontName='Helvetica-Bold'))
    styles.add(ParagraphStyle(name='CardTitle', fontSize=13, leading=16, alignment=TA_CENTER, textColor=colors.HexColor('#444444'), spaceAfter=10))
    styles.add(ParagraphStyle(name='StudentName', fontSize=14, leading=18, alignment=TA_LEFT, fontName='Helvetica-Bold'))
    styles.add(ParagraphStyle(name='CommentText', fontSize=10.5, leading=15, alignment=TA_LEFT, textColor=colors.HexColor('#333333'), spaceBefore=4))
    styles.add(ParagraphStyle(name='SectionHeader', fontSize=11.5, leading=14, alignment=TA_LEFT, fontName='Helvetica-Bold', textColor=colors.HexColor('#1F4E78'), spaceBefore=14, spaceAfter=6))
    return styles

def generate_grade_card_pdf(student_row: dict, module_rows: list, comment: str, output_dir: str = 'output/grade_cards/') -> str:
    out_path = Path(output_dir)
    out_path.mkdir(parents=True, exist_ok=True)
    filepath = out_path / f"{_safe_filename(student_row['email'])}.pdf"
    styles = _build_styles()
    doc = SimpleDocTemplate(str(filepath), pagesize=A4, topMargin=20*mm, bottomMargin=20*mm, leftMargin=20*mm, rightMargin=20*mm)
    story = []
    ...
    summary_data = [['Overall Average', 'Rank', 'Percentile', 'Grade'],
                    [f"{student_row['avg_percentage']}%", f"#{student_row['rank']}", f"{student_row['final_percentile']}", grade]]
    ...
    for m in module_rows:
        module_data.append([m['module'], f"{m['marks_scored']:.0f} / {m['marks_possible']:.0f}", f"{m['module_percentage']}%", f"{m['module_percentile']}"])
    ...
    story.append(Paragraph("Automatically generated by the Training Performance Management System.",
